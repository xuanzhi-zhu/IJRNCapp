...
% physical variables
global r_index m gra kp kv gamma kw M_beta_z K_beta_z e1 e2 e3 epsilon sigma eta delta M_beta_x M_beta_y K_beta_x K_beta_y avg_T para
...
%r_index=1,0<K_beta<sqrt(3)./3.*(gra-|r|), when r=1.2,
%0<K_beta<4.965
...
K_beta_y=1;%%para(26)
avg_T=0.01925;
% avg_T=0.1925;%%%%%%%%%%%%%%%%%%%%%%%%%%para(27) 0.0920(remain) 0.0716(switched)
para=[r_index;m;gra;kp;kv;gamma;kw;M_beta_z;K_beta_z;e1;e2;e3;epsilon;sigma;eta;delta;M_beta_x;M_beta_y;K_beta_x;K_beta_y;avg_T];

%%%===========================================================
% %%create initial points
% nPoints=1;
% [x_0_vec,inside_C_D_1]=fcn_initial_uniform_eta(nPoints,para);
...
x_0_vec=[x_0_vec;0];%%%%%%%%%%%%timer's initial condition
t_0=0;
No_cols=size(x_0_vec,2);
statistics_vec=zeros(7,No_cols);
for i=1:1:No_cols
    x_0=x_0_vec(:,i);
    sim quadrotor_20190317_small;
%     statistics_vec(:,i)=fcn_statistics(t,j,x);
end

%%%data_t_j_w_q_remain OR data_t_j_w_q_switched
t_w_q_remain_Discrete_controller=t;
j_w_q_remain_Discrete_controller=j;
save data_t_j_w_q_remain_Discrete_controller t_w_q_remain_Discrete_controller j_w_q_remain_Discrete_controller

% t_w_q_switched_Discrete_controller=t;
% j_w_q_switched_Discrete_controller=j;
% save data_t_j_w_q_switched_Discrete_controller t_w_q_switched_Discrete_controller j_w_q_switched_Discrete_controller